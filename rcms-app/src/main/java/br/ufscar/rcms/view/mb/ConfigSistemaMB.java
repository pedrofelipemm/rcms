...
public class ConfigSistemaMB extends AbstractMB {
    ...
    private void carregarConfiguracoes() {
		List<Configuracao> listaConfigs = getConfiguracaoService().buscarPorTipo(Configuracao.Tipo.getTiposConfigSistema());
    ...
    /*private void carregarNomeGrupo() {
    	if(getConfiguracao(Configuracao.Tipo.NOME_GRUPO) != null){
    		setNomeGrupo(getConfiguracao(Configuracao.Tipo.NOME_GRUPO).getValue());
    	}
    }

    private void carregarDescGrupo() {
    	if(getConfiguracao(Configuracao.Tipo.DESCRICAO_GRUPO) != null){
    		setDescGrupo(getConfiguracao(Configuracao.Tipo.DESCRICAO_GRUPO).getValue());
    	}
    }*/
    
    /*private void carregarLogotipo() {
    	if(getConfiguracao(Configuracao.Tipo.LOGOTIPO) != null){
    ...
    public Configuracao getConfiguracao(final Configuracao.Tipo tipo) {
        return configuracoes.stream().filter(c -> c.getKey().equals(tipo)).findFirst().orElse(new ConfiguracaoSistema(tipo));
    }
